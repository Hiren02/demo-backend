...
class Email {
	...
	async sendMail({ receiverMail, subject, senderEmail, text, transporterConfig, templateName, templateReplaceValue }) {
		try {
			...
			if (templateName) {
				...
				template = await promisify(ejs.renderFile)(templatePath, templateReplaceValue);
			}
			const mailOption = {
			...
			console.log("Preview URL: %s", nodemailer.getTestMessageUrl(sendingInfo));
		} catch (err) {
